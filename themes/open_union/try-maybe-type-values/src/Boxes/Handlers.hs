...
handleWithoutTime f reqs = withNextEvent f \case
	ButtonEvent { ev_event_type = 4, ev_button = eb } | Just b <- button eb ->
		pure $ expand (OccurredMouseDown [b] >- Empty :: EvOccs (Singleton MouseDown))
	...
	case getWait reqs of
		...
		_ -> do
			...
			(moccs :: Maybe (EvOccs GuiEv))  <- withNextEventTimeout' f (round $ dt * 1000000) \case
				Just ButtonEvent {
					...
					pure . Just $ expand (
						...
						(OccurredMouseDown [b] >- Empty :: EvOccs (Singleton MouseDown))
							:: UnionSet 'True (Occurred :$: (MouseMove :- Singleton MouseDown)))
				Just ButtonEvent {
					ev_event_type = 5, ev_button = eb,
					ev_x = x, ev_y = y } | Just b <- button eb ->
					pure . Just $ expand (
						OccurredMouseMove (fromIntegral x, fromIntegral y) >-
						(OccurredMouseUp [b] >- Empty :: EvOccs (Singleton MouseUp))
							:: UnionSet 'True (Occurred :$: (MouseMove :- Singleton MouseUp)))
				Just MotionEvent { ev_x = x, ev_y = y } ->
					pure . Just $ expand
						(OccurredMouseMove (fromIntegral x, fromIntegral y) >- Empty ::
			...
			case (moccs, mtoccs) of
				(Just occs, Just toccs) -> pure $ toccs `merge` occs
...
makeTimeObs :: EvReqs GuiEv -> DiffTime -> Maybe (UnionSet 'True (Occurred :$: TryWait :- DeltaTime :- 'Nil))
makeTimeObs r t = case (prj r, prj r) of
	(Just (TryWaitReq _t'), Just DeltaTimeReq) ->
		Just $ OccurredDeltaTime t >- (OccurredTryWait t >- Empty :: EvOccs (Singleton TryWait))
	(Just (TryWaitReq _t'), Nothing) ->
		Just $ expand (OccurredTryWait t >- Empty :: EvOccs (Singleton TryWait))
	(Nothing, Just DeltaTimeReq) -> Just $ expand (OccurredDeltaTime t >- Empty :: EvOccs (Singleton  DeltaTime))
