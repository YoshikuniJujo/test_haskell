...
Just (Wanted (Just wanted)  _) = expToWanted <$> parse bool "((p + d) == u)"

wanted2 :: Wanted1 String
Just (Wanted (Just wanted2) _) = expToWanted <$> parse bool "((n + nn) == (b + f))"
...
Just (Wanted (Just wanted3) _) = expToWanted <$> parse bool "((u + lm) == z)"
