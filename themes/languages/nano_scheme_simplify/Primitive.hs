...
import Eval (eval)
import Environment (
	Env, fromList, set, Value(..), showValue, Error(..),
...
list :: Value -> Either Error [Value]
list Nil = Right []; list (v `Cons` vs) = (v :) <$> list vs
list v = Left . Error $ prpLstErr ++ showValue v

nums :: [Value] -> Either Error (Either [Integer] [Double])
nums [] = Right $ Left []
nums (Int i : s) = either (Left . (i :)) (Right . (fromIntegral i :)) <$> nums s
nums (Dbl d : vs) = Right . (d :) . either (map fromIntegral) id <$> nums vs
nums (v : _) = Left . Error $ notNumErr ++ showValue v

add, mul, sub, divs, ltt :: Value -> Env -> Either Error (Value, Env)
...
sub v e = (, e) . either (Int . sb) (Dbl . sb) <$> (ck =<< nums =<< list v)
	where
	ck s | either null null s = Left $ Error lstOneErr | otherwise = Right s
...
stringAppend, x2string :: Value -> Env -> Either Error (Value, Env)
stringAppend (Cons (String str) strs) e = first (sa str) <$> stringAppend strs e
	where sa s (String ss) = String $ s ++ ss; sa _ _ = error "never occur"
