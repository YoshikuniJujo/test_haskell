...
main = do
	...
	void . Eff.runM . Except.run @String . Except.run @Zlib.ReturnCode
		...
		. Pipe.run
		. (`Except.catch` IO.putStrLn)
		. void $ fromImageGray IO img' (Header.headerToPoss' hdr')
--			Pipe.=$= PipeT.convert (GrayWord8 @Double <$>)
			Pipe.=$= PipeT.convert (word8ToBool <$>)
			Pipe.=$= Encode.encodeGray1 "barbaz" IO hdr' ibe obe
			Pipe.=$= PipeT.convert BSF.toStrict
			Pipe.=$= PipeBS.hPutStr hog
