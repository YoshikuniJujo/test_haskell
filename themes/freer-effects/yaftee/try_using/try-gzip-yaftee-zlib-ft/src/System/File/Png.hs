...
module System.File.Png (writePngGray, writePngGray') where
...
writePngGray'' fpp hdr (fctl, img) = do
	...
	hWritePngGray ho hdr (fctl, img) ibe obe
...
hWritePngGray ho hdr (fctl, img) ibe obe = do
	void . Eff.runM . Except.run @String . Except.run @Zlib.ReturnCode
		...
		. Pipe.run $ hWritePngPipeGray ho hdr fctl img ibe obe
...
hWritePngPipeGray ho hdr fctl img ibe obe = (`Except.catch` IO.putStrLn)
	. void $ fromFctlImagesGray IO hdr fctl img
		Pipe.=$= encodeApngGray hdr fctl ibe obe
...
encodeApngGray hdr fctl ibe obe =
...
encodeRawCalcGray :: forall nm m -> (
	...
	Eff.E es BodyGray BSF.ByteString ()
encodeRawCalcGray nm m hdr (w0, h0) mplt ibe obe = void $
-- MAKE IDAT
	pipeDat nm m hdr w0 h0 ibe obe
...
-- MAKE CHUNKS
	...
	Pipe.=$= do
		...
		Pipe.yield (Chunk { chunkName = "IEND", chunkBody = "" } ,)
	Pipe.=$= do
...
--	Pipe.=$= Buffer.format' nm BSF.splitAt' "" 100000
	Pipe.=$= PipeT.convert \dt sn' -> (Chunk "IDAT" dt, sn')
