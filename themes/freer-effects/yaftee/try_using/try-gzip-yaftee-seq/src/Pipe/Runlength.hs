...
runlength :: forall nm -> (U.Member Pipe.P es, Members nm es) =>
	Eff.E es R (Either Word8 (Seq.Seq Word8)) ()
runlength nm = fix \go -> Pipe.await >>= \rl -> ($ rl) \case
	Literal w -> (>> go)
		$ State.modifyN nm (`snoc` w) >> Pipe.yield (Left w)
	LiteralBS bs -> (>> go)
		$ State.modifyN nm (`appendR` bs) >> Pipe.yield (Right bs)
	LenDist ln d -> (>> go) $ State.getsN nm (repetition ln d) >>= \ws ->
		State.modifyN nm (`appendR` (Seq.fromList ws)) >> Pipe.yield (Right $ Seq.fromList ws)
	EndOfInput -> pure ()
...
appendR :: Seq -> Seq.Seq Word8 -> Seq
appendR (Seq s) ws = let s' = if ln > 32768 then Seq.drop (ln - 32768) s else s in
	Seq $ s' Seq.>< ws
--	Seq $ foldl (Seq.|>) s' ws
	where ln = Seq.length s

data R = Literal Word8 | LiteralBS (Seq.Seq Word8)
...
runLengthToLitLen (LiteralBS bs) = fromIntegral <$> toList bs
