...
generate w h px = G {
	width = w, height = h,
	body = V.generate (w' * h) \i -> b2w
		$ (<$> [0 .. 7]) \dx -> px' (i `mod` w' * 8 + dx) (i `div` w') }
	where
...
generateFromBytesM wdt hgt a = do
	bd <- V.unfoldrExactNM (((wdt - 1) `div` 8 + 1) * hgt)
		(\case	[] -> a >>= \case
				[] -> error "generateFromBytesM"
				(w : ws) -> pure (w, ws)
			w : ws -> pure (w, ws)) []
	pure G { width = wdt, height = hgt, body = bd }
