...
module Data.ByteString.Lazy.BitArray (

	-- * BIT ARRAY

	B, null, length, empty,

	-- * FROM/TO LAZY BYTE STRING
...
data B1 = B1 { zero :: Int64, length1 :: Int64, body :: LBS.ByteString }
	deriving Show
...
fromByteString :: LBS.ByteString -> B
fromByteString "" = B []
fromByteString bs = B [B1 { zero = 0, length1 = 8 * LBS.length bs, body = bs }]

toByteString :: B -> Either B LBS.ByteString
toByteString b@(B b1s) = case b1s of
	[] -> Right ""
	[b1] -> either (Left . B . (: [])) Right $ toByteString1 b1
	_ -> Left b

toByteString1 :: B1 -> Either B1 LBS.ByteString
...
appendByteString :: B -> LBS.ByteString -> B
		...
		b1'@B1 { zero = z2, length1 = ln2, body = bs2 } : b1s
			| (z1 + ln1) `mod` 8 == 0, z2 == 0 ->
				go B1 {	zero = z1, length1 = ln1 + ln2,
					body = bs1 `LBS.append` bs2 } b1s
			| (z1 + ln1 - z2) `mod` 8 == 0 ->
				go B1 {	zero = z1, length1 = ln1 + ln2,
					body = LBS.init bs1 `LBS.append` (
						(	LBS.last bs1 .&. bitMask 0 (z2 - 1) .|.
							LBS.head bs2 .&. bitMask z2 7) `LBS.cons`
						LBS.tail bs2 ) } b1s
			| otherwise -> b1 : go b1' b1s

bitMask :: Bits n => Int64 -> Int64 -> n
bitMask i j = foldl setBit zeroBits $ fromIntegral <$> [i .. j]

normalize1 :: B1 -> B1
normalize1 B1 { zero = z, length1 = ln, body = bs }
	| 0 <= z = B1 {
		zero = z',
		length1 = ln, body = LBS.take t $ LBS.drop (z `div` 8) bs }
...
byteBoundary1 b1@B1 { zero = z, length1 = ln, body = bs } = case z of
	...
	_	| z + ln <= 8 -> Nothing
		| otherwise -> Just (
			B1 { zero = z, length1 = 8 - z, body = LBS.take 1 bs },
			B1 { zero = 0, length1 = ln - 8 + z, body = LBS.tail bs } )

popBits :: Bits n => Int64 -> B -> Maybe (n, B)
popBits ln b0
...
toBits' b0
	| length b0 <= fromIntegral (finiteBitSize (undefined :: n)) = Just $ go 0 b0
	| otherwise = Nothing
	where go i b = case pop b of
		Nothing -> zeroBits
		Just (bt, bts) -> Bit.bit id (`setBit` i) bt $ go (i + 1) bts
