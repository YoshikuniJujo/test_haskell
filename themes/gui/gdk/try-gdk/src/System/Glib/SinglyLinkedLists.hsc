...
module System.Glib.SinglyLinkedLists where
...
mkGSList :: Ptr (GSList a) -> IO (GSList a)
mkGSList p = GSList <$> newForeignPtr p (c_g_slist_free p)

g_slist_uncons :: Ptr (GSList a) -> IO (Ptr a, Ptr (GSList a))
g_slist_uncons p = (,) <$> #{peek GSList, data} p <*> #{peek GSList, next} p

g_slist_to_list :: Ptr (GSList a) -> IO [Ptr a]
g_slist_to_list lst
	| lst == nullPtr = pure []
	| otherwise = do
