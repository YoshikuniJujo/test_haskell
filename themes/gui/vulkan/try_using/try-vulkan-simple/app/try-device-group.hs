...
import Gpu.Vulkan.Device.GroupDevice qualified as Vk.GDvc
...
withDvc a = Vk.Inst.create instInfo nil \inst -> do
	...
	Vk.Dvc.create pd (dvcInfo pd qfi) nil \dv ->
...
instInfo = def {
	Vk.Inst.createInfoApplicationInfo = Just ainfo,
...
dvcInfo :: Vk.Phd.P -> Vk.QFam.Index ->
	Vk.Dvc.CreateInfo ('Just (Vk.GDvc.CreateInfo 'Nothing)) '[ 'Nothing]
dvcInfo p qfi = Vk.Dvc.CreateInfo {
	Vk.Dvc.createInfoNext = TMaybe.J Vk.GDvc.CreateInfo {
		Vk.GDvc.createInfoNext = TMaybe.N,
		Vk.GDvc.createInfoPhysicalDevices = [p] },
