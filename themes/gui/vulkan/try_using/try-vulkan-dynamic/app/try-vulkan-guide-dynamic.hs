...
createBffrMem us pd dv gq cp
	...
	createBffrLst pd dv ln (Vk.Bffr.UsageTransferDstBit .|. us)
		...
		f (b, ln')
	where
	copy :: forall sm sb bnm sm' sb' bnm' al .
...
bffrAlgn :: forall o sd a . Obj.SizeAlignment o =>
	...
	(forall (al :: Nat) . KnownNat al => Proxy al -> IO a) -> IO a
bffrAlgn dv ln us f =
...
data BndVp alu bnmvp smsb where
	BndVp :: Vk.Bffr.Binded sm sb bnmvp '[AtmViewProj alu] ->
		BndVp alu bnmvp '(sm, sb)

data MemVp alu bnmvp smsb where
	MemVp ::
		Vk.Mm.M sm '[
			'(sb, 'Vk.Mm.BufferArg bnmvp '[AtmViewProj alu])] ->
...
createVpBffr :: Vk.Phd.P -> Vk.Dvc.D sd -> (forall sm sb .
...
createBffrAtm :: forall al sd nm a b . Storable a =>
...
createBffrLst :: forall al sd bnm lnm t a . Storable t =>
...
createDscSts :: (Vk.DscSt.DListFromMiddle sls, Update alu mff smsbs sls) =>
...
draw dv gq pq sc ex rp pl gp fbs
	vbmk vbtr vpms snm dss cbs (SyncObjs iass rfss iffs) cf fn =
