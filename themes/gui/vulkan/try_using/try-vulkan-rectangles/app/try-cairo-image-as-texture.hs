...
twoRectangles = do
	...
	cr <- cairoCreate sfc
	rectangle cr (0.7, 0.7, 0.7) 0 0 256 256
	rectangle cr (0.8, 0.2, 0.3) 50 50 110 110
	rectangle cr (0.7, 0.7, 0.3) 100 130 100 70
	cairoSurfaceFlush sfc
	cairoImageSurfaceGetCairoImage sfc >>= \case
		CairoImageArgb32 i -> pure i; _ -> error "never occur"
	where rectangle cr (r, g, b) l t w h = do
		cairoSetSourceRgb cr . fromJust $ rgbDouble r g b
		cairoRectangle cr l t w h >> cairoFill cr
...
recordCommandBuffer :: forall scb sr sf sl sg sm sb nm sm' sb' nm' sdsl sds alu alv nmv ali nmi .
