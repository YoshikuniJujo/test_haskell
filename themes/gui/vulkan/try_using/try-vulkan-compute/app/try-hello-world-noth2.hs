...
import Paths_try_vulkan_compute
...
calc dv q cpl dsl dss sz = compileShader >>= \shdr ->
	Vk.PplLyt.create dv (pplLytInfo dsl) nil \pl ->
	Vk.Ppl.Cmpt.createCs dv Nothing (HPList.Singleton . U4 $ pplInfo pl shdr)
		nil \(cppl :** HPList.Nil) ->
...
pplInfo :: Vk.PplLyt.P sl sbtss '[] -> SpirV.S GlslComputeShader ->
	Vk.Ppl.Cmpt.CreateInfo 'Nothing
		'( 'Nothing, 'Nothing, 'GlslComputeShader, 'Nothing, '[])
		'(sl, sbtss, '[]) sbph
pplInfo pl shdr = Vk.Ppl.Cmpt.CreateInfo {
	Vk.Ppl.Cmpt.createInfoNext = TMaybe.N,
	Vk.Ppl.Cmpt.createInfoFlags = zeroBits,
	Vk.Ppl.Cmpt.createInfoStage = U5 $ shdrStInfo shdr,
	Vk.Ppl.Cmpt.createInfoLayout = U3 pl,
	Vk.Ppl.Cmpt.createInfoBasePipelineHandleOrIndex = Nothing }

shdrStInfo :: SpirV.S GlslComputeShader -> Vk.Ppl.ShaderSt.CreateInfo
	'Nothing 'Nothing 'GlslComputeShader 'Nothing '[]
shdrStInfo shdr = Vk.Ppl.ShaderSt.CreateInfo {
	...
	where mdinfo = Vk.ShaderMod.CreateInfo {
		...
		Vk.ShaderMod.createInfoCode = shdr }
...
compileShader = do
	cd <- getDataFileName "shader/hello.comp" >>= BS.readFile
	Shaderc.compile @() cd "hello.comp" "main" def
