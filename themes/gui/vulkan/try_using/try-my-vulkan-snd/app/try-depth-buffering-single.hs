...
body txfp fr w ist =
	...
	createRndrPss @scifmt @dptfmt d \rp ->
	createPplLyt d \dsl pl ->
	...
	createImageView @'Vk.T.FormatR8g8b8a8Srgb d tximg Vk.Img.AspectColorBit \tximgvw ->
	...
	getCurrentTime >>= \tm ->
	mainLoop fr w sfc pd qfis d gq pq sc ex scvs rp pl gpl fbs cp dptImg dptImgMem dptImgVw vb ib ubm ubds cb sos tm
...
mkGraphicsPipelineCreateInfo' ::
	...
	Vk.Ppl.Graphics.CreateInfo 'Nothing '[
		...
		'(	'Nothing, '[ '(WVertex, 'Vk.VtxInp.RateVertex)],
			'[ '(0, Pos), '(1, Color), '(2, TexCoord)] )
		'Nothing 'Nothing 'Nothing 'Nothing 'Nothing 'Nothing 'Nothing 'Nothing '(sl, '[AtomUbo sdsl], '[]) sr '(sb, vs', ts', slbtss')
...
mainLoop :: (
	...
	Vk.RndrPss.R sr -> Vk.PplLyt.P sl '[AtomUbo sdsl] '[] ->
	Vk.Ppl.Graphics.G sg
		'[ '(WVertex, 'Vk.VtxInp.RateVertex)]
		'[ '(0, Pos), '(1, Color), '(2, TexCoord)]
		'(sl, '[AtomUbo sdsl], '[]) ->
