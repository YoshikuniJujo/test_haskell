...
type ColorResources nm fmt si sm siv = ClrRsrcs fmt nm si sm siv
...
instance Update _alm '[] '[] where update _ HPList.Nil HPList.Nil _ _ = pure ()
...
recordCommandBuffer :: forall scb sr sf sl sg sm sb nm sm' sb' nm' sdsl sds alm alv ali nmi .
...
mainLoop g w sfc phdvc qfis dvc gq pq sc ext0 scivs rp ppllyt gpl fbs cp crsrcs drsrcs vb (ib, idcs) cbs iasrfsifs ums dscss tm0 = do
	($ Inf.cycle $ NE.fromList [0 .. maxFramesInFlight - 1]) . ($ ext0) $ fix \loop ext (cf :~ cfs) -> do
		...
		runLoop w sfc phdvc qfis dvc gq pq
			sc g ext scivs rp ppllyt gpl fbs cp crsrcs drsrcs idcs vb ib cbs iasrfsifs ums dscss
...
drawFrame :: forall sfs sd ssc scfmt sr sl sdsc sg sm sb nm sm' sb' nm' scb ssos vss smsbs slyts sds alm alv ali nmi . (
	KnownNat alm, KnownNat alv, KnownNat ali,
	HPList.HomoList (AtomUbo sdsc alm) slyts,
	HPList.HomoList '() vss) =>
...
recreateSwapChainEtc :: (
	...
	DepthResources sdi sdm "depth-buffer" dptfmt sdiv ->
	IO Vk.Extent2d
recreateSwapChainEtc w@(GlfwG.Win.W win) sfc phdvc qfis dvc gq sc scivs rp ppllyt gpl fbs cp
