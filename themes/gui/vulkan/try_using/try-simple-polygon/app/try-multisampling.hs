...
body lb txi tctxi (vtcs, idcs) mnld fr w sfc (PhDvc.P pd qfis) =
	...
	tnum :: forall nmt a . Int -> (forall (n :: [()]) . (
		...
		CreateMvpBffrs nmt n ) => Proxy '(n, nmt) -> a) -> a
	tnum 0 f = f (Proxy @'( '[], nmt))
	tnum n f = tnum (n - 1) \p -> f $ plus1 p
		where plus1 :: Proxy '(n, nmt) -> Proxy '( '() ': n, nmt); plus1 Proxy = Proxy
...
createPplLyt :: forall alm sd nmt a . Vk.Dvc.D sd -> (forall sl sdsl .
	Vk.DscStLyt.D sdsl (DscStLytArg alm nmt) ->
...
createDscStLyt :: Vk.Dvc.D sd -> (forall (s :: Type) .
	Vk.DscStLyt.D s (DscStLytArg alm nmt) -> IO a) -> IO a
...
type BufferModelViewProj alm = 'Vk.DscStLyt.Buffer '[AtomModelViewProj alm]
...
createTxImg pd d gq cp img (mls, _, _) a = prepareImg pd d Vk.Img.TilingOptimal
		...
		w h mls Vk.Sample.Count1Bit \i m -> do
	createBffrImg pd d
...
recreateTxImg :: forall img sd sc inm a sm si . BObj.IsImage img =>
	Vk.Phd.P -> Vk.Dvc.D sd -> Vk.Q.Q -> Vk.CmdPl.C sc -> img ->
		Vk.Img.Binded sm si inm (BObj.ImageFormat img) ->
		Vk.Mm.M sm '[ '(si, 'Vk.Mm.ImageArg inm (BObj.ImageFormat img))] ->
		((Word32, Int32, Int32) -> IO a) -> IO ()
recreateTxImg pd d gq cp img i m a = reprepareImg' pd d Vk.Img.TilingOptimal
		(	Vk.Img.UsageTransferSrcBit .|.
			Vk.Img.UsageTransferDstBit .|.
			Vk.Img.UsageSampledBit ) Vk.Mm.PropertyDeviceLocalBit
		w h mls Vk.Sample.Count1Bit i m do
	createBffrImg pd d
		...
		\(b :: Vk.Bffr.Binded sm' sb inm '[bimg]) bm -> do
		Vk.Mm.write @inm @bimg @0 d bm zeroBits img
		transitionImageLayout d gq cp i Vk.Img.LayoutUndefined
			Vk.Img.LayoutTransferDstOptimal mls
		copyBffrToImg d gq cp b i
	a (mls, w, h)
	where
	mls = floor @Double . logBase 2 $ max w h
	w, h :: Num i => i
	w = fromIntegral $ BObj.imageWidth img
	h = fromIntegral $ BObj.imageHeight img
...
reprepareImg' :: Vk.T.FormatToValue fmt =>
	...
	Vk.Img.UsageFlags -> Vk.Mm.PropertyFlags -> Word32 -> Word32 ->
	Word32 -> Vk.Sample.CountFlags -> Vk.Img.Binded sm sb nm fmt ->
	Vk.Mm.M sm '[ '(sb, 'Vk.Mm.ImageArg nm fmt)] -> IO a -> IO ()
reprepareImg' pd dv tl us pr w h ml spcnt i m a =
	Vk.Img.unsafeRecreate' dv (imgInfo w h ml spcnt tl us) nil i do
	rqs <- Vk.Img.getMemoryRequirementsBinded dv i
	mt <- findMmType pd (Vk.Mm.requirementsMemoryTypeBits rqs) pr
	Vk.Mm.unsafeReallocateBind' dv
		(HPList.Singleton . U2 $ Vk.Mm.ImageBinded i)
		(memInfo mt) nil m a
...
generateMipmap1 :: forall scb sm si nm fmt . Vk.CBffr.C scb ->
...
generateMipmap1 cb img i w h = do
	...
	blit = Vk.Img.Blit {
		...
		Vk.Img.blitDstSubresource = sr i,
		Vk.Img.blitDstOffsetFrom = Vk.Offset3d 0 0 0,
		Vk.Img.blitDstOffsetTo = Vk.Offset3d (half w) (half h) 1 }
...
mipmapBarrier sam dam olyt nlyt img i = Vk.Img.MemoryBarrier {
	...
	Vk.Img.memoryBarrierSubresourceRange = Vk.Img.SubresourceRange {
...
createDscSts dv dp mbs dls tv tsp f =
	Vk.DscSt.allocateDs dv info $ (>>) <$> update dv mbs tv tsp <*> f
...
class Update nmt al smsbs slbtss where
	update :: Vk.Dvc.D sd -> HPList.PL (BindedModelViewProj al nm) smsbs ->
		...
		Vk.Smplr.S ssmp -> HPList.PL (Vk.DscSt.D sds) slbtss -> IO ()
...
dscWrite0 :: KnownNat alm => Vk.DscSt.D sds slbts ->
...
dscWrite1 :: Vk.DscSt.D sds slbts -> Vk.ImgVw.I nm fmt si -> Vk.Smplr.S ss ->
	Vk.DscSt.Write 'Nothing sds slbts
		('Vk.DscSt.WriteSourcesArgImage '[ '(ss, nm, fmt, si) ]) 0
dscWrite1 ds v s = Vk.DscSt.Write {
	Vk.DscSt.writeNext = TMaybe.N, Vk.DscSt.writeDstSet = ds,
...
mainloop :: (
	UpdateTx nmt alu slyts '(sdsl, DscStLytArg alu nmt), BObj.IsImage tximg,
	...
	ImgBffrs nmt smt sit tximg svt sspt -> UTCTime -> IO ()
...
type ImgBffrs nmt smt sit txi svt sspt = (
	Vk.Img.Binded smt sit nmt (BObj.ImageFormat txi),
	Vk.Mm.M smt '[ '(sit, 'Vk.Mm.ImageArg nmt (BObj.ImageFormat txi))],
	Vk.ImgVw.I nmt 'Vk.T.FormatR8g8b8a8Srgb svt, Vk.Smplr.S sspt )

run :: forall
	tximg sw ssfc sd sc scfmt ssc inm svs sr sl sdsl alu nmt sg sfs
	clrnm clrsi clrsm clrsiv sdi sdm dptfmt sdiv
	smv sbv bnmv alv nmv smi sbi bnmi ali nmi nmm smsbs
	sds sls scb mff ssoss smt sit svt sst . (
	BObj.IsImage tximg,
	UpdateTx nmt alu sls '(sdsl, DscStLytArg alu nmt),
	HPList.HomoList '() mff, RecreateFrmbffrs svs sfs,
	Vk.T.FormatToValue scfmt, Vk.T.FormatToValue dptfmt,
	HPList.HomoList '(sdsl, DscStLytArg alu nmt) sls,
	KnownNat alu, KnownNat alv, KnownNat ali ) =>
	TVar Glfw.MouseButtonState -> TChan () -> TChan tximg ->
	FramebufferResized -> GlfwG.Win.W sw -> Vk.Khr.Sfc.S ssfc ->
	Vk.Phd.P -> QFamIndices -> Vk.Dvc.D sd -> Vk.Q.Q -> Vk.Q.Q ->
	...
	SyncObjects ssoss -> Float -> Int ->
	ImgBffrs nmt smt sit tximg svt sst -> (Vk.Extent2d -> IO ()) -> IO ()
run lbst lb tctxi fr w sfc pd qfis dv gq pq cp
	sc ex vs rp pl gp fbs crs drs vb ib mms dss cbs soss tm cf
	(tx, txm, txv, txsp) go = do
	atomically (maybeReadTChan tctxi) >>= \case
		Nothing -> pure (); Just txi ->
			recreateTxImg pd dv gq cp txi tx txm \mwh@(mls, _, _) ->
			generateMipmaps pd dv gq cp tx mwh >>
			Vk.ImgVw.unsafeRecreate' dv
				(imgVwInfo tx Vk.Img.AspectColorBit mls) nil txv
				(updateTx @_ @alu @_
					@'(sdsl, DscStLytArg alu nmt)
					dv dss txv txsp)
	mouseButtonDown lbst w Glfw.MouseButton'1 >>=
		bool (pure ()) (atomically $ writeTChan lb ())
...
class UpdateTx nmt alu slbtss slbts where
	updateTx :: (
		'(sdsl, dsla) ~ slbts,
		...
		Vk.Dvc.D sd -> HPList.PL (Vk.DscSt.D sds) slbtss ->
		Vk.ImgVw.I nmt 'Vk.T.FormatR8g8b8a8Srgb siv ->
		Vk.Smplr.S ssmp -> IO ()

instance UpdateTx nmt alu '[] slbts where updateTx _ HPList.Nil _ _ = pure ()

instance UpdateTx nmt alu slbtss slbts =>
	UpdateTx nmt alu (slbts ': slbtss) slbts where
	updateTx dv (dss :** dsss) tv tsp = do
		Vk.DscSt.updateDs dv
			(HPList.Singleton . U5 $ dscWriteTx dss tv tsp)
			HPList.Nil
		updateTx @nmt @alu @slbtss @slbts dv dsss tv tsp

dscWriteTx :: Vk.DscSt.D sds slbts -> Vk.ImgVw.I nm fmt si -> Vk.Smplr.S ss ->
	Vk.DscSt.Write 'Nothing sds slbts
		('Vk.DscSt.WriteSourcesArgImage '[ '(ss, nm, fmt, si) ]) 0
dscWriteTx dss tv tsp = Vk.DscSt.Write {
	...
	Vk.DscSt.writeSources = Vk.DscSt.ImageInfos . HPList.Singleton
		$ U4 Vk.Dsc.ImageInfo {
			Vk.Dsc.imageInfoImageLayout =
				Vk.Img.LayoutShaderReadOnlyOptimal,
			Vk.Dsc.imageInfoImageView = tv,
			Vk.Dsc.imageInfoSampler = tsp } }

draw :: forall
	sfs sd ssc fmt sr sl sdsl sg
	smv sbv bnmv smi sbi bnmi scb ssos mff smsbs
	sls sds alu alv ali nmv nmi nmm nmt . (
	KnownNat alu, KnownNat alv, KnownNat ali,
	HPList.HomoList '() mff,
	HPList.HomoList '(sdsl, DscStLytArg alu nmt) sls ) =>
...
draw dv gq pq sc ex rp pl gp fbs
	vb ib mms dss cbs (SyncObjs iass rfss iffs) tm cf =
	...
	HPList.index fbs ii \fb -> recordCmdBffr cb rp fb ex pl gp vb ib ds
...
recordCmdBffr :: forall scb sr sf sl sg sm sb nm smi sbi bnmi sdsl sds alu alv ali nmv nmi nmt .
	...
	IO ()
recordCmdBffr cb rp fb sce ppllyt gpl vb ib ubds =
...
mouseButtonDown ::
	TVar Glfw.MouseButtonState -> GlfwG.Win.W sw -> Glfw.MouseButton -> IO Bool
mouseButtonDown st (GlfwG.Win.W w) b = do
...
updateUniformBuffer :: forall sd alu sm nmm . KnownNat alu =>
	Vk.Dvc.D sd -> MemoryUbo alu nmm sm -> Vk.Extent2d -> Float -> IO ()
updateUniformBuffer dvc (MemoryModelViewProj um) sce tm =
	Vk.Mm.write @nmm @(Obj.Atom alu WModelViewProj 'Nothing) @0 dvc um zeroBits ubo
...
recreateAll w@(GlfwG.Win.W win) sfc phdvc qfis dvc gq cp sc scivs rp ppllyt gpl
	...
	ext <$ do
		...
		recreateDptRsrcs phdvc dvc gq cp ext mss (dptImg, dptImgMem, dptImgVw)
...
maybeReadTChan c = bool (Just <$> readTChan c) (pure Nothing) =<< isEmptyTChan c
