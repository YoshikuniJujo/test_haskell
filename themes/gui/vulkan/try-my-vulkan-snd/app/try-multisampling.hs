	...
	(`withWindow` g) \win -> createInstance \inst -> do
		if enableValidationLayers
			then setupDebugMessenger inst
				$ const $ run txfp mdfp (read mnld) win inst g
...
mkHeteroParList _k [] f = f HeteroParList.HNil
...
createImageViews _dvc [] f = f HeteroParList.HNil
...
recreateImageViews _dvc [] HeteroParList.HNil = pure ()
...
createRenderPass dvc mss f = do
	let	colorAttachment :: Vk.Att.DescriptionNew scifmt
		colorAttachment = Vk.Att.DescriptionNew {
			...
			Vk.Att.descriptionSamplesNew = mss,
		...
		colorAttachmentResolve = Vk.Att.DescriptionNew {
			...
			Vk.Att.descriptionLoadOpNew = Vk.Att.LoadOpDontCare,
			Vk.Att.descriptionStoreOpNew = Vk.Att.StoreOpStore,
			Vk.Att.descriptionStencilLoadOpNew = Vk.Att.LoadOpDontCare,
			Vk.Att.descriptionStencilStoreOpNew =
				Vk.Att.StoreOpDontCare,
		...
		renderPassInfo = Vk.RndrPass.M.CreateInfoNew {
			...
			Vk.RndrPass.M.createInfoAttachmentsNew =
				...
				colorAttachmentResolve :** HeteroParList.HNil,
...
createDescriptorSetLayout dvc = Vk.DscSetLyt.create dvc layoutInfo nil nil
	...
	layoutInfo = Vk.DscSetLyt.CreateInfo {
		...
		Vk.DscSetLyt.createInfoBindings =
			uboLayoutBinding :**
			samplerLayoutBinding :** HeteroParList.HNil }
...
createPipelineLayout' dvc f =
	...
	let	pipelineLayoutInfo = Vk.Ppl.Layout.CreateInfoNew {
			...
			Vk.Ppl.Layout.createInfoSetLayoutsNew =
				HeteroParList.Singleton $ V2 dsl } in
...
createGraphicsPipeline' dvc sce rp ppllyt mss f =
	Vk.Ppl.Graphics.createGs dvc Nothing (V14 pplInfo :** HeteroParList.HNil)
			nil nil \(V2 gpl :** HeteroParList.HNil) -> f gpl
...
recreateGraphicsPipeline' dvc sce rp ppllyt mss gpls = Vk.Ppl.Graphics.recreateGs
	dvc Nothing (V14 pplInfo :** HeteroParList.HNil) nil nil (V2 gpls :** HeteroParList.HNil)
	where pplInfo = mkGraphicsPipelineCreateInfo' sce rp ppllyt mss
...
shaderStages :: HeteroParList.PL (V6 Vk.Ppl.ShdrSt.CreateInfoNew) '[
	'((), (), 'GlslVertexShader, (), (), '[]),
	'((), (), 'GlslFragmentShader, (), (), '[]) ]
shaderStages = V6 vertShaderStageInfo :** V6 fragShaderStageInfo :** HeteroParList.HNil
	where
...
createFramebuffers :: Vk.Dvc.D sd -> Vk.C.Extent2d ->
	...
	Vk.ImgVw.INew fmt clrnm slrsiv ->
	Vk.ImgVw.INew dptfmt dptnm siv ->
	(forall sfs . RecreateFramebuffers sis sfs =>
		HeteroParList.PL Vk.Frmbffr.F sfs -> IO a) -> IO a
createFramebuffers _ _ _ HeteroParList.HNil _ _ f = f HeteroParList.HNil
createFramebuffers dvc sce rp (iv :** ivs) clriv dptiv f =
	Vk.Frmbffr.createNew dvc (mkFramebufferCreateInfo sce rp iv clriv dptiv) nil nil \fb ->
...
class RecreateFramebuffers (sis :: [Type]) (sfs :: [Type]) where
	recreateFramebuffers :: Vk.Dvc.D sd -> Vk.C.Extent2d ->
		...
		Vk.ImgVw.INew scfmt clrnm clrsdiv ->
		Vk.ImgVw.INew dptfmt dptnm sdiv ->
		HeteroParList.PL Vk.Frmbffr.F sfs -> IO ()

instance RecreateFramebuffers '[] '[] where
	recreateFramebuffers _dvc _sce _rp HeteroParList.HNil _ _ HeteroParList.HNil = pure ()
...
instance RecreateFramebuffers sis sfs =>
	...
	recreateFramebuffers dvc sce rp (sciv :** scivs) clriv dptiv (fb :** fbs) =
		Vk.Frmbffr.recreateNew dvc
			(mkFramebufferCreateInfo sce rp sciv clriv dptiv) nil nil fb >>
...
mkFramebufferCreateInfo ::
	...
	Vk.ImgVw.INew fmt clrnm clrsdiv ->
	Vk.ImgVw.INew dptfmt dptnm sdiv ->
	Vk.Frmbffr.CreateInfoNew () sr
		'[ '(fmt, clrnm, clrsdiv), '(dptfmt, dptnm, sdiv), '(fmt, nm, si)]
mkFramebufferCreateInfo sce rp attch clr dpt = Vk.Frmbffr.CreateInfoNew {
	Vk.Frmbffr.createInfoNextNew = Nothing,
	Vk.Frmbffr.createInfoFlagsNew = zeroBits,
	Vk.Frmbffr.createInfoRenderPassNew = rp,
	Vk.Frmbffr.createInfoAttachmentsNew = V3 clr :** V3 dpt :** V3 attch :** HeteroParList.HNil,
...
createDepthResources ::
	...
	Vk.C.Extent2d -> Vk.Sample.CountFlags ->
...
createDepthResources phdvc dvc gq cp ext mss f = do
	...
	Vk.T.formatToType fmt \(_ :: Proxy fmt) -> do
		createImage @_ @fmt phdvc dvc
			(Vk.C.extent2dWidth ext) (Vk.C.extent2dHeight ext) 1 mss
...
createTextureImage ::
	Vk.PhDvc.P -> Vk.Dvc.D sd -> Vk.Queue.Q -> Vk.CmdPool.C sc -> FilePath -> (
		forall si sm .
		Vk.Img.BindedNew si sm nm 'Vk.T.FormatR8g8b8a8Srgb -> Word32 ->
		IO a ) -> IO a
...
generateMipmaps phdvc dvc gq cp img mlvs wdt hgt = do
	...
	beginSingleTimeCommands dvc gq cp \cb -> do
		...
		Vk.Cmd.pipelineBarrier cb
			...
			HeteroParList.HNil HeteroParList.HNil (HeteroParList.Singleton $ V5 barrier)
	where
...
transitionImageLayout :: forall sd sc si sm nm fmt . Vk.T.FormatToValue fmt =>
	Vk.Dvc.D sd -> Vk.Queue.Q -> Vk.CmdPool.C sc ->
	Vk.Img.BindedNew si sm nm fmt -> Vk.Img.Layout -> Vk.Img.Layout -> Word32 ->
	IO ()
...
createBuffer' p dv ln usg props f = Vk.Bffr.create dv bffrInfo nil nil \b -> do
	...
	Vk.Dvc.Mem.ImageBuffer.allocateBind dv (HeteroParList.Singleton . V2 $ Vk.Dvc.Mem.ImageBuffer.Buffer b)
		(allcInfo mt) nil nil
...
createUniformBuffers _ _ _ 0 f = f HeteroParList.HNil HeteroParList.HNil HeteroParList.HNil
...
instance Update '[] '[] ssmp siv where update _ HeteroParList.HNil HeteroParList.HNil _ _ = pure ()
...
instance VssList '[] where
	vssListIndex HeteroParList.HNil _ = error "index too large"
...
recordCommandBuffer cb rp fb sce ppllyt gpl idcs vb ib ubds =
	...
	Vk.Cmd.bindVertexBuffers cb
		. HeteroParList.Singleton . V4 $ Vk.Bffr.IndexedList @_ @_ @_ @Vertex vb
