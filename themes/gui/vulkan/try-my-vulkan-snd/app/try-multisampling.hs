...
run txfp mdfp w inst g =
	...
	createTextureSampler phdv dv mplvs \(txsmplr :: Vk.Smplr.S ssmp) ->
...
createBufferAtom :: forall sd nm a b . Storable a => Vk.PhDvc.P -> Vk.Dvc.D sd ->
	Vk.Bffr.UsageFlags -> Vk.Mem.PropertyFlags -> (
		forall sm sb .
		Vk.Bffr.Binded sb sm nm '[ 'Atom a 'Nothing] ->
		Vk.Dvc.Mem.ImageBuffer.M sm '[ '(
			sb,
			'Vk.Dvc.Mem.ImageBuffer.K.Buffer nm '[ 'Atom a 'Nothing] )] ->
			IO b) -> IO b
...
createIndexBuffer :: Vk.PhDvc.P ->
	Vk.Dvc.D sd -> Vk.Queue.Q -> Vk.CmdPool.C sc -> V.Vector Word32 -> (forall sm sb .
		Vk.Bffr.Binded sm sb "index-buffer" '[ 'List Word32 ""] -> IO a) -> IO a
		...
		\(b' :: Vk.Bffr.Binded sb sm "index-buffer" '[ 'List t ""])
			(bm' :: Vk.Dvc.Mem.ImageBuffer.M sm '[ '(
				...
				'Vk.Dvc.Mem.ImageBuffer.K.Buffer "index-buffer"
					'[ 'List Word32 ""] )]) -> do
	Vk.Dvc.Mem.ImageBuffer.write @"index-buffer" @('List Word32 "") dvc bm' zeroBits idcs
	copyBuffer dvc gq cp b' b
	f b
...
data BindedUbo smsb where
	BindedUbo :: Vk.Bffr.Binded sb sm "uniform-buffer" '[ 'Atom UniformBufferObject 'Nothing] ->
		BindedUbo '(sm, sb)
...
data MemoryUbo smsb where
	MemoryUbo :: Vk.Dvc.Mem.ImageBuffer.M sm '[ '(
		...
		'Vk.Dvc.Mem.ImageBuffer.K.Buffer "uniform-buffer"
			'[ 'Atom UniformBufferObject 'Nothing] )] ->
		MemoryUbo '(sm, sb)
...
updateUniformBuffer dvc (MemoryUbo um) sce tm =
	Vk.Dvc.Mem.ImageBuffer.write @"uniform-buffer" @('Atom UniformBufferObject 'Nothing) dvc um zeroBits ubo
