...
makeCommandBuffer device graphicsQueue cmdPool f = do
		...
		Vk.CommandBuffer.allocate device cmdBufAllocInfo \case
			[cmdBuf] -> do
				...
				Vk.Queue.submit graphicsQueue (V4 submitInfo :** HeteroParList.Nil) Nothing
...
makePipelineNew dvc rp f = do
	...
	Vk.Ppl.Lyt.createNew dvc layoutCreateInfoNew nil nil \plyt -> do
		let	pipelineCreateInfo :: Vk.Ppl.Gr.CreateInfo () '[
			...
			pipelineCreateInfo = Vk.Ppl.Gr.CreateInfo {
				...
				Vk.Ppl.Gr.createInfoStages =
					V6 vertShaderStage :**
					V6 fragShaderStage :** HeteroParList.Nil,
				...
				Vk.Ppl.Gr.createInfoLayout = V3 plyt,
		...
		Vk.Ppl.Gr.createGs dvc Nothing (
			V14 pipelineCreateInfo :** HeteroParList.Nil ) nil nil
				\(V2 g :** HeteroParList.Nil) -> f g
