...
run dvc qfam cmdBuf ppl pplLyt dscSet dsz rm memA memB memC = do
	...
	Vk.CmdBuf.beginNew @'Nothing @'Nothing cmdBuf def do
...
prepareMems33 phdvc dvc dscSetLyt da db dc f =
	...
	Vk.DscSet.updateDs @'Nothing @'Nothing dvc (U4
...
--	storage1BufferNew dvc phdvc da db dc \(buf' :: Vk.Buffer.B sb' nm' objs) bnd' m' ->
	...
	Vk.DscSet.allocateSs dvc (dscSetInfo dscPool dscSetLyt)
		>>= \(dscSet :** HeteroParList.Nil) ->
	Vk.DscSet.updateDs @'Nothing @'Nothing dvc (U4
		(writeDscSet' @w1 @w2 @w3 dscSet bufb) :** HeteroParList.Nil) [] >>
	f dscSet mib
...
shaderStageInfo :: Vk.Ppl.ShaderSt.CreateInfoNew 'Nothing 'Nothing 'GlslComputeShader sc () sd () '[]
