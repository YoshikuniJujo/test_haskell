...
run w inst g =
	...
	getCurrentTime >>= \tm ->
	mainLoop g w sfc phdv qfis dv gq pq sc ext scivs rp ppllyt gpl fbs cp (dptImg, dptImgMem, dptImgVw) vb ib cbs sos ubs ums dscss tm
	...
	let	colorAttachment :: Vk.Att.DescriptionNew scifmt
		...
		renderPassInfo = Vk.RndrPass.M.CreateInfoNew {
			...
			Vk.RndrPass.M.createInfoAttachmentsNew =
				colorAttachment :** depthAttachment :** HeteroParList.HNil,
...
createFramebuffers _ _ _ HeteroParList.HNil _ f = f HeteroParList.HNil
...
instance RecreateFramebuffers '[] '[] where
	recreateFramebuffers _dvc _sce _rp HeteroParList.HNil _ HeteroParList.HNil = pure ()
...
mkFramebufferCreateInfo sce rp attch dpt = Vk.Frmbffr.CreateInfoNew {
	...
	Vk.Frmbffr.createInfoAttachmentsNew = V3 attch :** V3 dpt :** HeteroParList.HNil,
...
createVertexBuffer phdvc dvc gq cp f =
	...
	Vk.Dvc.Mem.ImageBuffer.write @"vertex-buffer" @('List 256 Vertex "") dvc bm' zeroBits vertices
	copyBuffer dvc gq cp b' b
	f b

createIndexBuffer :: Vk.PhDvc.P ->
	Vk.Dvc.D sd -> Vk.Queue.Q -> Vk.CmdPool.C sc -> (forall sm sb .
		Vk.Bffr.Binded sm sb "index-buffer" '[ 'List 256 Word16 ""] -> IO a) -> IO a
		...
		\(b' :: Vk.Bffr.Binded sb sm "index-buffer" '[ 'List 256 t ""])
			(bm' :: Vk.Dvc.Mem.ImageBuffer.M sm '[ '(
				...
				'Vk.Dvc.Mem.ImageBuffer.K.Buffer "index-buffer"
					'[ 'List 256 Word16 ""] )]) -> do
	Vk.Dvc.Mem.ImageBuffer.write @"index-buffer" @('List 256 Word16 "") dvc bm' zeroBits indices
	copyBuffer dvc gq cp b' b
	f b
...
mainLoop :: (
	...
	UTCTime ->
	IO ()
mainLoop g w sfc phdvc qfis dvc gq pq sc ext0 scivs rp ppllyt gpl fbs cp drsrcs vb ib cbs iasrfsifs ubs ums dscss tm0 = do
	($ cycle [0 .. maxFramesInFlight - 1]) . ($ ext0) $ fix \loop ext (cf : cfs) -> do
		Glfw.pollEvents
		tm <- getCurrentTime
		runLoop w sfc phdvc qfis dvc gq pq
			sc g ext scivs rp ppllyt gpl fbs cp drsrcs vb ib cbs iasrfsifs ubs ums dscss
