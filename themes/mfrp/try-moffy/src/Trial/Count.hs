...
tryLeftCount = do
	...
	interpretReact (retry $ handle Nothing f) (leftCount 0) <* closeField f
...
tryLeftCountSig = do
	...
	interpret (retry $ handle Nothing f) print (leftCountSig 0) <* closeField f
...
tryLeftRandomSig = do
	...
	interpret (retry $ handle Nothing f) print (leftRandomSig $ mkStdGen 8) <* closeField f
