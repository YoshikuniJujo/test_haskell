...
xkJapaneseKeyboard = concat <$> zipWithM (flip mkXk) ([0xff21 .. 0xff30] ++ [0xff37, 0xff3e, 0xff3e]) [
...
xkCursorControlAndMotion = concat <$> zipWithM (flip mkXk) [0xff50 .. 0xff58] [
	"Home", "Left", "Up", "Right", "Down", "Prior", "Next", "End", "Begin"
	]

xkMiscFunctions :: DecsQ
xkMiscFunctions = concat <$> zipWithM (flip mkXk) ([0xff60 .. 0xff6b] ++ [0xff7e, 0xff7f]) [
	"Select", "Print", "Excecute", "Insert", "Undo", "Redo", "Menu", "Find", "Cancel",
	"Help", "Break", "Mode_switch", "Num_Lock" ]

xkF1ToF35 :: DecsQ
xkF1ToF35 = concat <$> zipWithM mkXk (("F" ++) . show <$> [1 :: Int .. 35]) [0xffbe .. 0xffe0]
...
xkModifiers :: DecsQ
xkModifiers = concat <$> zipWithM (flip mkXk) [0xffe1 .. 0xffee] [
