...
import Stopgap.Graphics.UI.Gtk.TfeTextView qualified as Tfe.TextView
...
beforeClose :: Gtk.ApplicationWindow.A -> Gtk.Notebook.N -> IO Bool
beforeClose _win nb = do
	n <- Gtk.Notebook.getNPages nb
	for_ [0 .. n - 1] \i -> do
--		scr <- Gtk.Notebook.getNthPage nb i
...
appOpen app files _hint Null = do
	...
	if nps > 0
	then do	G.Signal.connectClose win "close-request" beforeClose nb
		Gtk.Window.present win
	else Gtk.Window.destroy win
