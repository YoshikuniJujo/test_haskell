...
summary = [\t -> do
	writeTopTitle t "まとめ"
	text t "", \t -> do
	text t "* リストは「くりかえし」の実体化である", \t -> do
	text t "* 簡易的に使えるデータ構造としての側面もある", \t -> do
	text t "* Haskellでは標準的な文字列は文字のリストである", \t -> do
	text t "* データ構造としてのリストは使いどころを選ぶ", \t -> do
	itext t 1 "- 大量の要素の保存には向かない", \t -> do
	itext t 1 "- ランダムアクセスには向かない", \t -> do
	itext t 1 "- プロトタイプであればそれらにも使うこともある"
 ]
