...
リストを扱う再帰関数の多くは、要素間に同一の演算子を挿入した形となる。

演算子の適用のしかたには右結合と左結合とがある。

そのような構造は関数によって抽象化することができ、
実際に標準ライブラリで以下の関数が定義されている。
...
再帰的に定義すれば、どの関数を作ることができる。
以下の関数については演習で扱うことにする。

    take, drop, concat, reverse, map, filter

演習では、それぞれの関数の再帰的な定義を見ていき、
可能なものについてはfoldrやfoldlによる定義も行うことにする。

まとめ
------

* リストの名部構造を学び、パターンマッチのやりかたを見た。
* リストを扱う再帰関数の定義のしかたを学んだ。
* リストを扱う関数の多くがあてはまる枠組みを見た
    + 要素間に演算子を入れるという枠組みであり
    + それには右結合と左結合とがある
* それらは標準ライブラリに定義されている
    + foldr: 右結合で演算子を挿入する
    + foldl: 左結合で演算子を挿入する
