...
* 'a'からは'a' -> 'b', 'a' -> 'c'という経路がある
* 'b'からは'b' -> 'd', 'b' -> 'e'という経路がある
* 'e'からは'e' -> 'f', 'e' -> 'g'という経路がある

### 経路の存在関数

#### どのような関数か

経路が存在するかどうかを調べる関数existPathを作る。
できあがったときには以下のような動作をする。

    existPath paths 'b' 'f' => True
    existPath paths 'c' 'g' => False
    existPath paths 'f' 'b' => False

#### 型

まずは型を決める。
使用例から明らかなように入力、出力は以下のようになるだろう。
...
まずは基底ケースを考える。
最も簡単なケースとして始点と終点が同じ場合が考えられる。

この場合は経路は存在すると考えてよいので
...
もうひとつの基底ケースがある。
bとeが同一でなく、しかもbからの経路がない場合である。

この場合は経路は存在しないと考えられるので
...
bからの経路がないということは、
psのなかにbを1番目の要素とするタプルが存在しないということ。

よって、lookup b psはNothingを返す。

##### 再帰部

残るのはbとeが同一ではなくbからの経路が存在する可能性である。

この場合、bのふたつの直接の子のどちらかから、
eへの経路が存在すればbからeへの経路は存在し、
そうでなければbからeへの経路は存在しない。

つまり、bからeへの経路が存在するというBool値は、
bの子1からeへの経路が存在するというBool値と
bの子2からeへの経路が存在するというBool値との論理和となる。
...
![tree](treeImage.png "medium")

    *Main> :reload
    *Main> existPath paths 'b' 'f'
    True
    *Main> existPath paths 'c' 'g'
    False
    *Main> existPath paths 'f' 'b'

木を下にしかたどれないという条件を思い出そう。
関数は正しい結果を返している。

まとめ
------

再帰関数の定義のしかた/理解のしかたについて学んだ。

定義する際には、基底ケースを見つけ、
基底に近い場合の値からより離れた場合の値を導く。

理解する際には、それが「何であるか」をよく見る。
また、逐次的に評価してみる。
そして、その結果得た仮説を検証する。
