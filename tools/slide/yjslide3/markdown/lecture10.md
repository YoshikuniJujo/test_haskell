...
* 演習10-14. zipの型を決めよ(1分)
* 演習10-15. 再帰を直接使ってzipを定義せよ(1分)
* 演習10-16. unfoldrを使ってzipを定義せよ(1分)
...
関数unzipはほぼzipの逆関数となる。
2要素タプルのリストを2つのリストに分ける関数である。
2つのリストはタプルにまとめて返される。

    Prelude> unzip [('y', 1), ('e', 2), ('s', 3)]
    ("yes",[1,2,3])

### 演習

* 演習10-17. unzipの型を求めよ
* 演習10-18. 再帰を直接使ってunzipを定義せよ
* 演習10-19. foldrを使ってunzipを定義せよ
...
関数zipWithは2つのリストの要素同士を、タプルにまとめるのではなく、
任意の関数でまとめる。
...
* 演習10-20. zipWithの型を求めよ(1分)

### zip

タプルは(x, y)で表記されるが本当は以下の形をしている。

    (,) x y

つまり二引数関数(,)をx, yに適用している。

* 演習10-21. zipWithを使ってzipを定義せよ(30秒)

### 定義

* 演習10-22. 再帰を直接使ってzipWithを定義せよ(1分)
* 演習10-23. unfoldrを使ってzipWithを定義せよ(1分)

まとめ
------

いろいろな再帰関数を定義した。
とくに、リストを扱う再帰関数について、直接的な定義を作成した。
また、抽象的な枠組みを抽出した関数を使って定義した。

これらにより、再帰的関数の定義のしかたを身につけた。
また、より狭い枠組みを抽出した関数を使うことで、
定義しようとしている関数のパターンを見出した。
