...
mkToLife l nt@(h : t) =
	...
	to = mkName $ n ++ "To" ++ l
	tol = mkName $ "to" ++ l
mkToLife _ _ = error "bad"

mkFromLifeTyp :: String -> String -> DecQ
mkFromLifeTyp l (h : t) =
	sigD from $ forallT [PlainTV $ mkName "a"]
		(cxt [classP life [varT $ mkName "a"]]) $
			conT sm `arrT` (conT ''Maybe `appT` varT (mkName "a"))
	where
	n = toLower h : t
	from = mkName $ n ++ "From" ++ l
	life = mkName l
	sm = mkName $ "Some" ++ l
mkFromLifeTyp _ _ = error "bad"
