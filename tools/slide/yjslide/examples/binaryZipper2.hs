...
goLeft (Node x l r, logs) = Just (l, Log Left x r : logs)
goLeft _ = Nothing

goRight (Node x l r, logs) = Just (r, Log Right x l : logs)
goRight _ = Nothing
-- goLeft (Node x l r, logs) = (l, (Left, x, r) : logs)
-- goRight (Node x l r, logs) = (r, (Right, x, l) : logs)

goUp (t, Log Left x r : logs) = Just (Node x t r, logs)
goUp (t, Log Right x l : logs) = Just (Node x l t, logs)
goUp _ = Nothing
...
topMost z@(_, []) = z
topMost z = topMost $ fromJust $ goUp z

fromTree :: Tree a -> Maybe (Zipper a)
fromTree t = Just (t, [])

toTree :: Maybe (Zipper a) -> Tree a
toTree (Just z) = let (t, []) = topMost z in t
toTree _ = Empty

modify :: (a -> a) -> Zipper a -> Maybe (Zipper a)
modify f (Node x l r, logs) = Just (Node (f x) l r, logs)
modify _ (Empty, logs) = Just (Empty, logs)

put :: a -> Zipper a -> Maybe (Zipper a)
put x (Node _ l r, logs) = Just (Node x l r, logs)
put x (Empty, logs) = Just (Node x Empty Empty, logs)
