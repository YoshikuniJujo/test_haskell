...
instance Lazy (Parser a (b -> b)) where
        defer g = Parser \s -> runParser (g unit) s
...
recL1 :: forall a b . (b -> b -> b) -> Parser a b -> Parser a b
recL1 f p = (\x xs -> xs x) <$> p <*> recL1' f p

recL1' :: forall a b c . (b -> c -> b) -> Parser a c -> Parser a (b -> b)
recL1' f p =
        ((\x xs y -> xs (f y x)) <$> p <*> defer (\_ -> recL1' f p))
        `alt`
        pure id
