...
//		samplerInfo.minLod = 0.0f;
	...
	void createTextureImage() {
		int texWidth, texHeight, textChannels;
		stbi_uc* pixels = stbi_load(
			TEXTURE_PATH.c_str(), &texWidth, &texHeight,
...
/*
	...
	void generateMipmaps(
		...
		for (uint32_t i = 1; i < mipLevels; i++) {
			...
			auto& ssr = blit.srcSubresource;
			ssr.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			ssr.mipLevel = i - 1;
			...
			auto& dsr = blit.dstSubresource;
			dsr.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			dsr.mipLevel = i;
			dsr.baseArrayLayer = 0;
			dsr.layerCount = 1;
	...
	void transitionImageLayout(
		...
		srr.baseMipLevel = 0; srr.levelCount = mipLevels;
	...
	void createDescriptorSets() {
		...
		if (vkAllocateDescriptorSets(device,
			&allocInfo, descriptorSets.data() ) != VK_SUCCESS) {
			throw std::runtime_error(
	...
	void createDescriptorPool() {
		std::cout << "CREATE DESCRIPTOR POOL BEGIN" << std::endl;
		...
		std::cout << "CREATE DESCRIPTOR POOL END" << std::endl;
	}
	...
	void createIndexBuffer() {
		std::cout << "CREATE INDEX BUFFER BEGIN" << std::endl;
	...
	void createBuffer(
		...
		std::cout << std::endl << "VK MEMORY REQUIREMENTS" << std::endl;
