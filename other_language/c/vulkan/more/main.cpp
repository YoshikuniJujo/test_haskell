...
private:
	...
	void createTextureImage() {
		...
		imageInfo.extent.width = static_cast<uint32_t>(textWidth);
		...
		if (vkCreateImage(
			device, &imageInfo, nullptr, &textureImage )
			!= VK_SUCCESS) {
			throw std::runtime_error(
		...
		vkGetImageMemoryRequirements(
			device, textureImage, &memRequirements );
		...
		if (vkAllocateMemory(device,
			&allocInfo, nullptr, &textureImageMemory)
			!= VK_SUCCESS) {
			throw std::runtime_error(
	...
	void createDescriptorSets() {
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			vkUpdateDescriptorSets(
				device, 1, &descriptorWrite, 0, nullptr );
		}
	}
