...
module IHDR (
	IHDR(..), ColorType(..), HasColor(..), HasAlpha(..), frChunk, toChunk
	) where
...
toColorType w = case w `popBits` 3 of
	...
	([False, hc, ha], 0) ->
		Just $ NoPalette (toEnum $ fromEnum hc) (toEnum $ fromEnum ha)
	_ -> Nothing
