...
instance HFunctor.HFunctorO R where
	hmapO _ g (Ask k) = Ask $ g . k

-- instance HFunctor.HFunctorI R where
--	hmapI _ g (Ask k) = Ask $ g . k
...
run :: HFunctor.HFunctorI (Union.U effs) =>
...
m `run` i = ($ m) $ fix \go -> \case
	...
	u HFreer.:>>= q -> case Union.decomp u of
		Left u' -> HFunctor.hmapI (`run` i) Identity u' HFreer.:>>=
