...
bind (Writer x w) f = let Writer y w' = f x in Writer y (w ++. w')

instance Functor Writer where
	f `fmap` Writer x w = Writer (f x) w
...
sampleL = {-# SCC "LeftAssociatedHellos" #-} foldl (>=>) pure $ replicate 8000 sampleFun
sampleR = {-# SCC "WriteAssoociatedHellos" #-} foldr (>=>) pure $ replicate 8000 sampleFun
