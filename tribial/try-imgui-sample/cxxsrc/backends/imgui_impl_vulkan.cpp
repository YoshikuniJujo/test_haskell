...
VkSurfaceFormatKHR* ImGui_ImplVulkanH_SelectSurfaceFormat2(VkPhysicalDevice physical_device, VkSurfaceKHR surface, const VkFormat* request_formats, int request_formats_count, VkColorSpaceKHR request_color_space)
{
	static VkSurfaceFormatKHR ret;
	ret = ImGui_ImplVulkanH_SelectSurfaceFormat(physical_device, surface, request_formats, request_formats_count, request_color_space);
	return &ret;
}
...
VkCommandPool *ImGui_ImplVulkanH_CreateWindowCommandBuffersCreateCommandPool(
	...
	uint32_t ic )
{
	printf("*** ImGui_ImplVulkanH_CreateWindowCommandBuffersCreateCommandPool begin ***\n");

    // Create Command Buffers
    VkResult err;
    VkCommandPool *cps;

    cps = (VkCommandPool *)malloc(sizeof(VkCommandPool) * ic);
    ...
    for (uint32_t i = 0; i < ic; i++)
    {
		...
		err = vkCreateCommandPool(device, &info, allocator, &cps[i]);
            check_vk_result(err);
        }
    }
	...
	ImGui_ImplVulkanH_Window* wd, VkCommandPool *cps )
{
    ...
    for (uint32_t i = 0; i < ic; i++)
    {
        ...
        {
		memcpy(&fd->CommandPool, &cps[i], sizeof(VkCommandPool));
        }
    }
}
...
{
    for (uint32_t i = 0; i < sc; i++)
        ImGui_ImplVulkanH_DestroyFrameSemaphores(device, &smps[i], allocator);
    smps.clear();
}
